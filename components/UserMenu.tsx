...
interface UserMenuProps {
    ...
    onAIAdministration?: () => void;
}

export const UserMenu: React.FC<UserMenuProps> = ({ user, onTourManagement, onShoutoutManagement, onAIAdministration }) => {
        ...
        <div className="relative" ref={menuRef}>
            ...
            {isOpen && (
                <div className="absolute right-0 mt-2 w-64 bg-white rounded-lg shadow-lg border border-slate-200 py-2 z-50">
                    ...
                    {/* Admin Options */}
                    {isAdmin && (onTourManagement || onShoutoutManagement || onAIAdministration) && (
                        <div className="px-4 py-3 border-b border-slate-200">
                            <div className="space-y-2">
                                ...
                                {onAIAdministration && (
                                    <button
                                        ...
                                        className="w-full px-3 py-2 text-sm font-medium text-green-600 hover:text-green-700 hover:bg-green-50 rounded-md transition-colors duration-200 flex items-center justify-center space-x-2"
                                    >
                                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z" />
                                        </svg>
                                        <span>{t('aiAdministration')}</span>
                                    </button>
                                )}
                            </div>
                        </div>
